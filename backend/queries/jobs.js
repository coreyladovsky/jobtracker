...
const getAllUserJobs = async (req, res, next) => {
    try {
        const jobs = await db.any("SELECT * FROM jobs WHERE user_id = $1", req.user.id)
        res.json({
            jobs, 
            message: "All Jobs"
        })
...
const updateJob = async (req, res, next) => {
 try {
   await db.none(
     "UPDATE jobs SET company=${company}, job_title=${job_title}, post_url=${post_url}, location=${location}, salary=${salary}, due_date=${due_date}, description=${description}, status=${status} WHERE user_id = ${user_id}",
     req.body
   );
   res.json({
     message: "Job Updated",
